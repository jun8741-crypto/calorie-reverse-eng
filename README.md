...
| 역할 | 대회 · 맥락 | 기간 | 성과 |
|---|---|---|---|
| **팀장 (3명, 13조)** | AI 헬스케어 ML 해커톤 | 2026.02 | 리더보드 1위 (비공식) · RMSE 0.5 → 0.02 |

> **TL;DR** 트리 모델(CatBoost)로 RMSE ~0.5 정체 → 데이터 생성 메커니즘을 의심 → Keytel(2005) 칼로리 소모 공식과 체중 단위 버그(정수 kg이 lb로 저장)를 역공학으로 복원 → 공식 직접 적용으로 RMSE 0.02(기수 내 리더보드 1위, 비공식) 달성.
